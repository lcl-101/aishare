...
if __name__ == "__main__":
    ...
    print(f"🔄 开始转换微调模型...")
    print(f"📂 源路径: {checkpoint}")
    print(f"💾 目标路径: {whisper_dump_path}")
    
    # 检查checkpoint是否存在
    import os
    if not os.path.exists(checkpoint):
        print(f"❌ 检查点不存在: {checkpoint}")
        print("请确保训练已完成并生成了checkpoint-4000")
        exit(1)
    
    # 确保models目录存在
    os.makedirs(os.path.dirname(whisper_dump_path), exist_ok=True)
    
    try:
        convert_tfms_to_openai_whisper(checkpoint, whisper_dump_path)
        print(f"✅ 模型转换完成！")
        print(f"📁 OpenAI格式模型已保存至: {whisper_dump_path}")
        print(f"🎯 现在可以使用 whisper.load_model('{whisper_dump_path}') 加载模型")
