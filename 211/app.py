...
# 强制使用 V0 engine 避免 CUDA fork 问题
os.environ['VLLM_USE_V1'] = '0'

# 设置多进程启动方式为 'spawn' 以避免 CUDA 重新初始化错误
try:
...
class SeedXTranslator:
    ...
    def load_all_models(self):
        ...
        try:
            # H20 显存充足，可以同时加载两个模型
            memory_per_model = 0.4  # 每个模型使用40%显存
            ...
            for i, (model_name, model_path) in enumerate(self.model_paths.items()):
                ...
                model = LLM(
                    ...
                    enable_prefix_caching=False,  # 禁用 prefix caching（与 sliding window 不兼容）
                    gpu_memory_utilization=memory_per_model,
                    trust_remote_code=True,
                    enforce_eager=True,
                    disable_log_stats=True,
                    disable_async_output_proc=True  # 禁用异步输出处理
                )
