...
class DreamOPipeline(FluxPipeline):
    def __init__(self, scheduler, vae, text_encoder, tokenizer, text_encoder_2, tokenizer_2, transformer, quantize_type=None):
        ...
        if quantize_type in ["int8", "int4"]:
            self._quantize_transformer(self.transformer, quantize_type)

    def _quantize_transformer(self, module, quantize_type):
        for name, child in module.named_children():
            if isinstance(child, nn.Linear):
                ...
                self._quantize_transformer(child, quantize_type)

    def load_dreamo_model(self, device, use_turbo=True):
        # hf_hub_download(repo_id='ByteDance/DreamO', filename='dreamo.safetensors', local_dir='checkpoints')
        # hf_hub_download(repo_id='ByteDance/DreamO', filename='dreamo_cfg_distill.safetensors', local_dir='checkpoints')
        dreamo_lora = load_file('checkpoints/DreamO/dreamo.safetensors')
        cfg_distill_lora = load_file('checkpoints/DreamO/dreamo_cfg_distill.safetensors')
        ...
        if use_turbo:
            self.load_lora_weights(
                "checkpoints/FLUX.1-Turbo-Alpha/diffusion_pytorch_model.safetensors",
                adapter_name='turbo',
            )
