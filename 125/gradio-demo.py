...
whisper_model = whisper.load_model("base")  # 你可以改为 "small" "medium" "large"
inpainter = PlayDiffusion()

def run_asr(audio):
    # audio: 文件路径
    result = whisper_model.transcribe(audio, word_timestamps=True, verbose=False)
    # result["segments"] 是分段，segments 里有 words 字段
    word_times = []
