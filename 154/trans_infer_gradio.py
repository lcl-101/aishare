...
MODEL_PATH = "checkpoints/GLM-4.1V-9B-Thinking"
...
class GLM4VModel:
    ...
    def stream_generate(self, raw_hist, sys_prompt):
        ...
        try:
            ...
            yield f"Error processing input: {str(e)}"
            return
...
def chat(files, msg, raw_hist, sys_prompt):
    ...
    if not ok:
        ...
        # 保持文件和预览状态
        image_preview, video_preview = update_preview(files)
        yield display_hist, copy.deepcopy(raw_hist), files, "", image_preview, video_preview
        return
...
def reset():
    ...
    time.sleep(0.1)
    return (
        [], [], None, "",  # chatbox, raw_history, up, textbox
        gr.update(visible=False, value=None),  # image_preview
        gr.update(visible=False, value=None)   # video_preview
    )
...
def update_preview(files):
    ...
    for f in files:
        ...
        elif ext in [".mp4", ".avi", ".mkv", ".mov", ".wmv", ".flv", ".webm", ".mpeg", ".m4v"]:
            video_file = f.name
            break  # 只显示第一个视频
    
    # 返回图片预览和视频预览的更新
    if image_files:
        image_update = gr.update(visible=True, value=image_files[0])
        video_update = gr.update(visible=False, value=None)
    elif video_file:
        image_update = gr.update(visible=False, value=None)  
        video_update = gr.update(visible=True, value=video_file)
    else:
        image_update = gr.update(visible=False, value=None)
        video_update = gr.update(visible=False, value=None)
    
    return image_update, video_update
...
with demo:
    ...
    gr.on(
        ...
        outputs=[chatbox, raw_history, up, textbox, image_preview, video_preview],
    )
    clear.click(
        reset, 
        outputs=[chatbox, raw_history, up, textbox, image_preview, video_preview]
