...
model = None
processor = None
current_ckpt_path = None  # 新增：记录当前已加载的模型路径

def _load_model_processor(args, ckpt_path=None):
    if args.cpu_only:
        device_map = 'cpu'
    else:
        device_map = 'auto'
    ...
    if args.flash_attn2:
        ...
        model = Qwen2_5OmniForConditionalGeneration.from_pretrained(
            ckpt_path, device_map=device_map, torch_dtype='auto')
...
def _launch_demo(args):
    ...
    MODEL_CHOICES = [
        ("checkpoints/Qwen2.5-Omni-3B", "Qwen2.5-Omni-3B"),
        ("checkpoints/Qwen2.5-Omni-7B", "Qwen2.5-Omni-7B"),
    ]

    def get_model_and_processor(selected_ckpt_path=None, force_reload=False):
        global model, processor, current_ckpt_path
        # 检查是否需要加载或切换模型
        if (model is None or processor is None or
            (selected_ckpt_path and selected_ckpt_path != current_ckpt_path) or force_reload):
            if model is not None or processor is not None:
                print(f"Unloading model: {current_ckpt_path}")
                del model
                del processor
                gc.collect()
                model = None
                processor = None
            ckpt_path = selected_ckpt_path or global_args.checkpoint_path
            print(f"Loading model and processor: {ckpt_path}")
            model_, processor_ = _load_model_processor(global_args, ckpt_path)
            model = model_
            processor = processor_
            current_ckpt_path = ckpt_path
        return model, processor

    def predict(messages, voice=DEFAULT_VOICE, selected_ckpt_path=None):
        print('predict history: ', messages)
        # 延迟加载模型，支持模型切换
        model_, processor_ = get_model_and_processor(selected_ckpt_path=selected_ckpt_path)

        text = processor_.apply_chat_template(messages, add_generation_prompt=True, tokenize=False)

        audios, images, videos = process_mm_info(messages, use_audio_in_video=False)

        inputs = processor_(text=text, audio=audios, images=images, videos=videos, return_tensors="pt", padding=True, use_audio_in_video=False)
        inputs = inputs.to(model_.device).to(model_.dtype)

        text_ids, audio = model_.generate(**inputs, speaker=voice, use_audio_in_video=False)

        response = processor_.batch_decode(text_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)
    ...
    def media_predict(audio, video, history, system_prompt, voice_choice, selected_ckpt_path):
        # First yield
        yield (
            None,  # microphone
            None,  # webcam
        ...
        for chunk in predict(formatted_history, voice_choice, selected_ckpt_path):
            if chunk["type"] == "text":
    ...
    def chat_predict(text, audio, image, video, history, system_prompt, voice_choice, selected_ckpt_path):
        # Process text input
        if text:
    ...
    with gr.Blocks() as demo, ms.Application(), antd.ConfigProvider():
        with gr.Sidebar(open=False):
            ...
            model_selector = gr.Dropdown(
                choices=[x[0] for x in MODEL_CHOICES],
                ...
                with gr.Row():
                    ...
                    submit_event = submit_btn.click(
                        ...
                        inputs=[
                            ...
                            system_prompt_textbox, voice_choice, model_selector
                        ],
                        outputs=[
                            microphone, webcam, media_chatbot, submit_btn, stop_btn
                        ])
                    stop_btn.click(
                        fn=lambda:
...
DEFAULT_CKPT_PATH = "checkpoints/Qwen2.5-Omni-3B"
def _get_args():
