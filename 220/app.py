...
def run_model(
    ...
    if model is None:
        model = WorldMirror.from_pretrained("checkpoints/HunyuanWorld-Mirror").to(device)
    else:
        model.to(device)
...
with gr.Blocks(
    ...
    <div style="font-size: 16px; line-height: 1.5;">
        <p>WorldMirror 支持任意组合的输入（图像、相机内参、相机位姿、深度），并且可以一次性输出点云、相机参数、深度图、法线图以及 3D 高斯球（3DGS）等多种结果。</p>
    <h3>使用步骤：</h3>
    <ol>
        <li><strong>上传数据：</strong> 点击 “Upload Video or Images” 按钮添加文件。若上传视频，系统会按一秒间隔自动抽帧。</li>
        <li><strong>开始重建：</strong> 点击 “Reconstruct” 按钮，启动 3D 重建流程。</li>
            <li><strong>结果可视化：</strong> 在不同的标签页中查看多种重建结果：
                <ul>
                    <li><strong>3D View：</strong> 带有相机位姿的交互式点云/网格视图（可下载 GLB）</li>
                    <li><strong>3D Gaussian Splatting：</strong> 交互式 3DGS 视图，并提供 RGB/深度视频（可下载 PLY）</li>
                    <li><strong>Depth Maps：</strong> 每一视角的深度估计结果（可下载 PNG）</li>
                    <li><strong>Normal Maps：</strong> 每一视角的表面法线可视化（可下载 PNG）</li>
                    <li><strong>Camera Parameters：</strong> 估计的相机位姿和内参（可下载 JSON）</li>
                </ul>
            </li>
    </ol>
    <p><strong style="color: #3b82f6;">温馨提示：加载数据和展示 3D 效果可能需要一些时间。若想获得更快体验，建议从 GitHub 下载代码后在本地运行。</strong></p>
    </div>
    """)
