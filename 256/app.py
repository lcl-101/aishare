...
# ============================================================
# 预先导入 torch 以确保 PyTorch 扩展模块能正确加载
# 这样可以确保 libc10.so 等库被正确加载
# ============================================================
import os
import sys
import torch  # 必须在导入 custom_rasterizer 之前导入 torch

# ============================================================
# Mock bpy module to avoid ImportError
# bpy (Blender Python API) 在普通 Python 环境中不可用
# 我们创建一个假的 bpy 模块，这样导入不会报错
# 实际的 OBJ 转 GLB 功能使用 trimesh 实现，不依赖 bpy
# ============================================================
from types import ModuleType

class MockBpy(ModuleType):
    """Mock bpy module that raises error only when actually used for Blender operations"""
    
    # 这些属性需要返回安全值，避免在模块检查时报错
    __file__ = None
    __path__ = []
    __all__ = []
    __version__ = "mock"
    ...
    def __getattr__(self, name):
        # 对于模块检查相关的属性，返回 None 而不是报错
        if name.startswith('_'):
            return None
        raise ImportError(
            "bpy (Blender Python API) is not available. "
            "This app uses trimesh for OBJ to GLB conversion instead."
        )

# 在导入其他模块之前，将 mock bpy 注入到 sys.modules
if 'bpy' not in sys.modules:
    sys.modules['bpy'] = MockBpy('bpy')
...
# ============================================================
# 配置本地模型路径（已预下载到 checkpoints 目录）
# ============================================================
# 获取当前脚本所在目录
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# 本地模型路径配置
LOCAL_MODEL_PATH = os.path.join(SCRIPT_DIR, "checkpoints/Hunyuan3D-2.1")
LOCAL_SUBFOLDER = "hunyuan3d-dit-v2-1"
LOCAL_TEXGEN_MODEL_PATH = os.path.join(SCRIPT_DIR, "checkpoints/Hunyuan3D-2.1")
...
def _gen_shape(
    ...
    if image is None:
        ...
        try:
            ...
            Please enable it by `python app.py --enable_t23d`.")
                    ...
                    with gr.Tab('Advanced Options', id='tab_advanced_options'):
                        ...
                        with gr.Row():
                            num_steps = gr.Slider(maximum=100,
                                                  minimum=1,
                                                  value=5 if 'turbo' in SUBFOLDER else 30,
...
if __name__ == '__main__':
    ...
    # 默认使用本地模型路径
    parser.add_argument("--model_path", type=str, default=LOCAL_MODEL_PATH,
                        help="模型路径，默认使用本地 checkpoints/Hunyuan3D-2.1")
    parser.add_argument("--subfolder", type=str, default=LOCAL_SUBFOLDER,
                        help="子文件夹名称，默认 hunyuan3d-dit-v2-1")
    parser.add_argument("--texgen_model_path", type=str, default=LOCAL_TEXGEN_MODEL_PATH,
                        help="纹理生成模型路径，默认使用本地 checkpoints/Hunyuan3D-2.1")
    ...
    # 打印配置信息
    print("=" * 60)
    print("Hunyuan3D-2.1 配置信息:")
    print(f"  模型路径: {MODEL_PATH}")
    print(f"  子文件夹: {SUBFOLDER}")
    print(f"  纹理模型路径: {TEXGEN_MODEL_PATH}")
    print(f"  低显存模式: {LOW_VRAM_MODE}")
    ...
    if not args.disable_tex:
        try:
            ...
            # ============================================================
            # Patch huggingface_hub.snapshot_download 以支持本地路径
            # ============================================================
            import huggingface_hub
            _original_snapshot_download = huggingface_hub.snapshot_download
            
            def _patched_snapshot_download(repo_id, **kwargs):
                """如果 repo_id 是本地路径，直接返回；否则调用原函数下载"""
                if os.path.isdir(repo_id):
                    print(f"Using local model path: {repo_id}")
                    return repo_id
                return _original_snapshot_download(repo_id, **kwargs)
            ...
            # 使用本地模型路径，避免从 HuggingFace 下载
            conf.multiview_pretrained_path = TEXGEN_MODEL_PATH
            # 使用本地 DINOv2 模型
            conf.dino_ckpt_path = os.path.join(SCRIPT_DIR, "checkpoints/dinov2-giant")
