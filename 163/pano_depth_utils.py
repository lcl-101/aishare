...
def build_depth_model(device: torch.device = "cuda"):
    ...
    """
    # Try to load model from local path first, then fallback to online
    try:
        # First try to load from online (this should work with proper model format)
        print("Attempting to load MoGe model from Hugging Face Hub...")
        model = MoGeModel.from_pretrained("Ruicheng/moge-vitl")
        print("Successfully loaded MoGe model from Hugging Face Hub")
    except Exception as e:
        print(f"Failed to load from Hugging Face Hub: {e}")
        try:
            # Fallback: try local path
            print("Attempting to load MoGe model from local checkpoint...")
            model = MoGeModel.from_pretrained("checkpoints/moge-vitl")
            print("Successfully loaded MoGe model from local checkpoint")
        except Exception as e2:
            print(f"Failed to load from local checkpoint: {e2}")
            raise RuntimeError(f"Could not load MoGe model from either online or local sources. Online error: {e}, Local error: {e2}")
