...
class LayerDecomposition():
    ...
    def __init__(self):
        ...
        self.ratio = 1.5
        self.grounding_model = "checkpoints/grounding-dino-tiny"
        self.zim_model_config = "vit_l"
        self.zim_checkpoint = "./ZIM/zim_vit_l_2092"  # Add zim anything ckpt here
        self.inpaint_model = "checkpoints/FLUX.1-Fill-dev"
        self.inpaint_fg_lora = "checkpoints/HunyuanWorld-1"
        self.inpaint_sky_lora = "checkpoints/HunyuanWorld-1"
        self.scale = 2
        ...
        # Initialize model references but don't load them all at once
        self.sr_model = None
        self.zim_predictor = None
        self.gd_processor = None
        self.gd_model = None
        self.inpaint_fg_model = None
        self.inpaint_sky_model = None
        
        # Load only essential models first (lighter models)
        print("=============  load Super-Resolution models ====")
        self.sr_model = sr_utils.build_sr_model(scale=self.scale, gpu_id=0)
        print("=============  load Super-Resolution models done")
        
        print("=============  load Segmentation models ========")
        ...
        # Mark inpaint models as not loaded yet
        self._inpaint_models_loaded = False

    def _load_inpaint_models(self):
        """Load inpaint models on demand to save memory"""
        if not self._inpaint_models_loaded:
            print("=============  load panorama inpaint models ====")
            import gc
            import torch
            ...
            self._inpaint_models_loaded = True
    ...
    def __call__(self, input, layer):
        ...
        # Ensure inpaint models are loaded before use
        self._load_inpaint_models()
