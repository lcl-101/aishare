...
_HF_DIR = os.path.abspath(os.path.dirname(__file__))
_BASE_DIR = os.path.abspath(os.path.join(_HF_DIR, ".."))

# Allow environment variable overrides; otherwise use local repo structure
# Expected layout:
#   models/Wan2.1-Fun-1.3B-InP/
#   weights/transformer/
#   configs/wan2.1/wan_civitai.yaml
pretrained_model_path = os.environ.get(
    "ROSE_PRETRAINED_MODEL_PATH",
    os.path.join(_BASE_DIR, "models", "Wan2.1-Fun-1.3B-InP")
)
transformer_path = os.environ.get(
    "ROSE_TRANSFORMER_PATH",
    os.path.join(_BASE_DIR, "weights", "transformer")
)
config_path = os.environ.get(
    "ROSE_CONFIG_PATH",
    os.path.join(_BASE_DIR, "configs", "wan2.1", "wan_civitai.yaml")
)

if not os.path.exists(config_path):
    raise FileNotFoundError(f"Config file not found at {config_path}. You can set ROSE_CONFIG_PATH to override.")
config = OmegaConf.load(config_path)

# ---------------- Path resolution helpers ---------------- #
def _resolve_path(subpath: str, must_exist: bool = True):
    """Try several locations for a given model subpath.
    Order:
      1. If subpath is absolute and exists -> return
      2. <pretrained_model_path>/<subpath>
      3. <_BASE_DIR>/models/<subpath>
      4. <_BASE_DIR>/models/Wan2.1-Fun-1.3B-InP/<subpath> (legacy)
    """
    candidates = []
    if os.path.isabs(subpath):
        candidates.append(subpath)
    candidates.append(os.path.join(pretrained_model_path, subpath))
    candidates.append(os.path.join(_BASE_DIR, 'models', subpath))
    candidates.append(os.path.join(_BASE_DIR, 'models', 'Wan2.1-Fun-1.3B-InP', subpath))
    for c in candidates:
        if os.path.exists(c):
            print(f"[ROSE][path-resolve] Using '{subpath}' -> {c}")
            return c
    if must_exist:
        raise FileNotFoundError(f"None of the candidate paths exist for '{subpath}':\n" + "\n".join(candidates))
    return candidates[0]

# ---------------- Load components with fallback ---------------- #
_text_enc_cfg = config['text_encoder_kwargs']
text_encoder_subpath = _text_enc_cfg.get('text_encoder_subpath', 'text_encoder')
tokenizer_subpath = _text_enc_cfg.get('tokenizer_subpath', 'tokenizer')

tokenizer_path = _resolve_path(tokenizer_subpath)
text_encoder_path = _resolve_path(text_encoder_subpath)

_image_enc_cfg = config['image_encoder_kwargs']
image_encoder_subpath = _image_enc_cfg.get('image_encoder_subpath', 'image_encoder')
image_encoder_path = _resolve_path(image_encoder_subpath)

_vae_cfg = config['vae_kwargs']
vae_subpath = _vae_cfg.get('vae_subpath', 'vae')
vae_path = _resolve_path(vae_subpath)

tokenizer = AutoTokenizer.from_pretrained(tokenizer_path)
text_encoder = WanT5EncoderModel.from_pretrained(
    text_encoder_path,
    additional_kwargs=OmegaConf.to_container(_text_enc_cfg),
    low_cpu_mem_usage=True,
)
if not os.path.exists(text_encoder_path):  # defensive (should already exist)
    print(f"[ROSE][warning] text encoder path not found at first attempt: {text_encoder_path}")
    # force fallback to root models for single-file checkpoints
    alt_path = os.path.join(_BASE_DIR, 'models', os.path.basename(text_encoder_path))
    if os.path.exists(alt_path):
        print(f"[ROSE][fallback] retry loading text encoder from {alt_path}")
        text_encoder = WanT5EncoderModel.from_pretrained(
            alt_path,
            additional_kwargs=OmegaConf.to_container(_text_enc_cfg),
        ...
        raise FileNotFoundError(f"Text encoder checkpoint not found. Tried {text_encoder_path} and {alt_path}")
clip_image_encoder = CLIPModel.from_pretrained(image_encoder_path)
vae = AutoencoderKLWan.from_pretrained(
    vae_path,
    additional_kwargs=OmegaConf.to_container(_vae_cfg),
)
...
iface.queue()
iface.launch(debug=True, server_name="0.0.0.0", server_port=args.port)
