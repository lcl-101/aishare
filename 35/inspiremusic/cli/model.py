...
class InspireMusicModel:
    ...
    def llm_job(self, text, audio_token, audio_token_len, prompt_text, llm_prompt_audio_token, embeddings, uuid, duration_to_gen, task):
        with self.llm_context:
            local_res = []
            # Force torch.float16 for FlashAttention
            with autocast(enabled=self.fp16, dtype=torch.float16, cache_enabled=True):
                inference_kwargs = {
    ...
    def token2wav(self, token, token_len, uuid, sample_rate, finalize=False, flow_cfg=None):
        # Ensure token indices are within valid range to avoid out-of-bound errors.
        token = token.long().clamp(0, self.flow.vocab_size - 1)
            ...
            if self.fast:
                ...
                if self.llm:
                    ...
                    if len(self.music_token_dict[this_uuid]) != 0:
                        ...
                        print(f"The list of tensors is empty for UUID: {this_uuid}, 使用text_token作为默认值")
                        this_music_token = text_token  # 后备赋值，避免变量未定义报错
                else:
                    this_music_token = text_token
                if 'this_music_token' in locals():
                ...
                # If trimming is enabled and tokens are beyond minimum hop length, trim them
                if trim and (embeddings[1] - embeddings[0]) <= duration_to_gen:
                    if this_music_token.size(1) < self.token_min_hop_len:
                        logging.warning("Generated token length is too short; skipping trimming to avoid indexing errors.")
                    else:
                ...
                # Use fp16 for computation if enabled (required for FlashAttention)
                with autocast(enabled=self.fp16, dtype=torch.float16 if self.fp16 else self.dtype, cache_enabled=True):
